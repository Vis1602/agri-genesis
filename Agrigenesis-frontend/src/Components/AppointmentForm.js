import React, { useState, useEffect } from "react";
import { Link } from "react-router-dom";
import "../Styles/AppointmentForm.css";
import { ToastContainer, toast } from "react-toastify";
import { appointmentService } from "../services/appointmentService";
import api from "../services/api";
import Navbar from "./Navbar";

function AppointmentForm() {
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "smooth" });
    // Load experts when component mounts
    fetchExperts();
  }, []);

  const [FarmerName, setFarmerName] = useState("");
  const [FarmerNumber, setFarmerNumber] = useState("");
  const [issue, setIssue] = useState("");
  const [appointmentTime, setAppointmentTime] = useState("");
  const [preferredMode, setPreferredMode] = useState("default");
  const [selectedExpert, setSelectedExpert] = useState("default");
  const [experts, setExperts] = useState([]);
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [formErrors, setFormErrors] = useState({});

  const fetchExperts = async () => {
    try {
      const experts = await api.get("/users/experts").then((res) => res.data);
      setExperts(experts);
    } catch (error) {
      toast.error("Failed to load experts. Please try again.");
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();

    // Validate form inputs
    const errors = {};
    if (!FarmerName.trim()) {
      errors.FarmerName = "Farmer name is required";
    }

    if (!FarmerNumber.trim()) {
      errors.FarmerNumber = "Farmer phone number is required";
    } else if (FarmerNumber.trim().length !== 10) {
      errors.FarmerNumber = "Farmer phone number must be of 10 digits";
    }

    if (!issue.trim()) {
      errors.issue = "Please describe your issue";
    }

    if (!appointmentTime) {
      errors.appointmentTime = "Appointment time is required";
    } else {
      const selectedTime = new Date(appointmentTime).getTime();
      const currentTime = new Date().getTime();
      if (selectedTime <= currentTime) {
        errors.appointmentTime = "Please select a future appointment time";
      }
    }

    if (preferredMode === "default") {
      errors.preferredMode = "Please select preferred mode";
    }

    if (selectedExpert === "default") {
      errors.selectedExpert = "Please select an expert";
    }

    if (Object.keys(errors).length > 0) {
      setFormErrors(errors);
      return;
    }

    try {
      // Create appointment
      await appointmentService.createAppointment({
        expertId: selectedExpert,
        appointmentDate: appointmentTime,
        mode: preferredMode,
        issue,
        number: FarmerNumber,
      });

      toast.success("Appointment Scheduled!", {
        position: toast.POSITION.TOP_CENTER,
        onOpen: () => setIsSubmitted(true),
        onClose: () => setIsSubmitted(false),
      });

      // Reset form fields and errors
      setFarmerName("");
      setFarmerNumber("");
      setIssue("");
      setAppointmentTime("");
      setPreferredMode("default");
      setSelectedExpert("default");
      setFormErrors({});
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Failed to schedule appointment"
      );
    }
  };

  return (
    <>
      <Navbar />
      <div className="appointment-form-section">
        <h1 className="appointment-siteTitle">
          <Link to="/">AgriGenesis</Link>
        </h1>

        <div className="form-container">
          <h2 className="form-title">
            <div>Book Appointment Online</div>
          </h2>

          <form className="form-content" onSubmit={handleSubmit}>
            <label>
              Farmer Full Name:
              <input
                type="text"
                value={FarmerName}
                onChange={(e) => setFarmerName(e.target.value)}
                required
              />
              {formErrors.FarmerName && (
                <p className="error-message">{formErrors.FarmerName}</p>
              )}
            </label>

            <br />
            <label>
              Farmer Phone Number:
              <input
                type="text"
                value={FarmerNumber}
                onChange={(e) => setFarmerNumber(e.target.value)}
                required
              />
              {formErrors.FarmerNumber && (
                <p className="error-message">{formErrors.FarmerNumber}</p>
              )}
            </label>

            <br />
            <label>
              Select Expert:
              <select
                value={selectedExpert}
                onChange={(e) => setSelectedExpert(e.target.value)}
                required
              >
                <option value="default">Select</option>
                {experts.map((expert) => (
                  <option key={expert._id} value={expert._id}>
                    {expert.name} - {expert.specialization}
                  </option>
                ))}
              </select>
              {formErrors.selectedExpert && (
                <p className="error-message">{formErrors.selectedExpert}</p>
              )}
            </label>

            <br />
            <label>
              Describe Your Issue:
              <textarea
                value={issue}
                onChange={(e) => setIssue(e.target.value)}
                required
                rows={4}
              />
              {formErrors.issue && (
                <p className="error-message">{formErrors.issue}</p>
              )}
            </label>

            <br />
            <label>
              Preferred Appointment Time:
              <input
                type="datetime-local"
                value={appointmentTime}
                onChange={(e) => setAppointmentTime(e.target.value)}
                required
              />
              {formErrors.appointmentTime && (
                <p className="error-message">{formErrors.appointmentTime}</p>
              )}
            </label>

            <br />
            <label>
              Preferred Mode:
              <select
                value={preferredMode}
                onChange={(e) => setPreferredMode(e.target.value)}
                required
              >
                <option value="default">Select</option>
                <option value="voice">Voice Call</option>
                <option value="video">Video Call</option>
              </select>
              {formErrors.preferredMode && (
                <p className="error-message">{formErrors.preferredMode}</p>
              )}
            </label>

            <br />
            <button type="submit" className="text-appointment-btn">
              Confirm Appointment
            </button>

            <p
              className="success-message"
              style={{ display: isSubmitted ? "block" : "none" }}
            >
              Appointment details have been sent to your phone number via SMS.
            </p>
          </form>
        </div>

        

        <ToastContainer autoClose={5000} limit={1} closeButton={false} />
      </div>
    </>
  );
}

export default AppointmentForm;
